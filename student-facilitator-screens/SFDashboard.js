  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const checkerAttendanceUrl = `http://192.168.86.39:8000/api/faci-attendance?studentId=${encodeURIComponent(username)}`;
        ...
        if (!dutiesResponse.ok || !attendanceResponse.ok || !checkerAttendanceResponse.ok)
            ...
            {stat.progress !== undefined && (
              <View style={styles.progressContainer}>
                <View style={styles.progressBar}>
                  <View
                    style={[styles.progressFill, { width: `${stat.progress}%`, backgroundColor: stat.color }]}
                  />
                </View>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 13, fontWeight: '500', marginBottom: 4, color: '#475569', textAlign: 'center' },
